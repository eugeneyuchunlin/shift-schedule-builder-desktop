...
class Solver(object):
    ...
    def _solve(self, per_grave, n1, k, num_sweeps, year, month, lmda,
                lmdb, lmdc, lmdd, lmde):
        ...
        X_initial = Array.create("Graveyard", shape = per_grave*days, vartype = "BINARY")
        ...
        Hb = 0
        cycle = len(shift_kd[0]) # the cycle of shift_kd would be (per_grave, days - k)
        print(cycle)
        slack = Array.create("slack", shape = (per_grave,cycle*k), vartype = "BINARY")
        for i in range(per_grave):
            for j in range(cycle):
                # To adapt the condition k is not equal to 4, a for-loop needed
                Hb = Hb + (shift_kd[i][j] - sum(slack[i][l] for l in range(k*j, k*j+k)) )**2
        ...
        for i in range(per_grave):
            ...
            Hc = Hc + (And(X[i][0], Not(X[i][1]))) + (And(X[i][days-1], Not(X[i][days-2])))


        ## a weekday leave

        He = 0
        week_slack = Array.create("slack2", shape = (per_grave,days), vartype = "BINARY")

        for j in range(per_grave):
            for i in weekend[::2]:
                if i + 7 < days:
                    He = He + (sum(X[j][l] for l in range(i, i+7)) - sum(week_slack[j][l] for l in range(i, i+5)))**2
                elif i + 5 < days and i + 7 > days:
                    He = He + (sum(X[j][l] for l in range(i, days)) - sum(week_slack[j][l] for l in range(i, i+5)))**2
        ...
        label = ['per_grave', 'shift_grave', 'num_var'  ,'energy', 'weekdayleave', 'eachshift', 'kdays', '2days', 'process_time', 'num_sweeps']
        ...
        graveyard_dic = {graveyard_list[i]:graveyard_table[i].tolist() for i in range(per_grave)}
